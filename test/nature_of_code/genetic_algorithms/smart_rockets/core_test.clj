(ns nature-of-code.genetic-algorithms.smart-rockets.core-test
  (:use 
    clojure.test)
  (:require 
    [quil.core :as q]
...
(defn gen-test-world [] 
  (let [dna (smart-rockets/random-dna 3)
        ...
        obstacles []] ; TODO ?
    (smart-rockets/gen-world :population population :obstacles obstacles :target [100 100])))
  ...
  (with-redefs [smart-rockets/params test-params] 
    (testing 
      ...
      (is
        (= 
          false
          (smart-rockets/containso? (Obstacle. [50 50] 50  50) [0 0])))
      (is 
        (=
         true
         (smart-rockets/containso? (Obstacle. [50 50] 50 50) [75 75]))))))
  ...
  (testing 
    ...
    (is 
      (test-utils/close-to 
        0.0
        (:fitness
          (smart-rockets/fitness (smart-rockets/gen-rocket :location [100 100] :fitness 0)))))))
    ...
    (testing 
      ...
      (is 
        (pos? ; greater than zero
              (get-in
                (let [test-world (gen-test-world)]
                  (smart-rockets/calc-rockets-fitness (:population test-world) (:target test-world)))
                [:rockets 0 :fitness]))))
    (testing 
