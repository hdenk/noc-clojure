  ...
  (testing 
    ...
    (is 
      (= 
        ...
        (select-keys 
          (seek-agent/next-state (gen-vehicle :location (PVector. 1 1) :velocity (PVector. 0.1 0.2)))
          [:location])))
    (is
      (= 
        {:velocity (PVector. 0.1 0.2)}  
         (select-keys 
           (seek-agent/next-state (gen-vehicle :velocity (PVector. 0.0 0.0) :acceleration (PVector. 0.1 0.2) :max-speed 1.0))
           [:velocity])))
    (is
      (= 
        {:velocity (PVector. 0.0 0.1)}  
         (select-keys 
           (seek-agent/next-state (gen-vehicle :velocity (PVector. 0.0 0.0) :acceleration (PVector. 0.0 0.3) :max-speed 0.1))
           [:velocity])))
    (is
      (= 
        {:velocity (PVector. 0.1 0.0)}  
         (select-keys 
           (seek-agent/next-state (gen-vehicle :velocity (PVector. 0.0 0.0) :acceleration (PVector. 0.3 0.0) :max-speed 0.1))
           [:velocity])))
    (is
      (= 
        {:acceleration (PVector. 0 0)}  
         (select-keys 
           (seek-agent/next-state (gen-vehicle :acceleration (PVector. 0.1 0.2)))
           [:acceleration])))
    (is 
      (=  
        (select-keys 
        ...
        (select-keys 
          (seek-agent/next-state (gen-vehicle :location (PVector. 1 1) :velocity (PVector. 0.1 0.2) :acceleration (PVector. 0.1 0.2) :max-speed 1.0))
          [:id :mass :location :velocity :acceleration :r :max-speed :max-force]))))
  (testing 
    "apply-force"
    (is 
